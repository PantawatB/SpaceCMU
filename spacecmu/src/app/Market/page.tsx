  ...
  return (
    <article className="bg-white rounded-xl shadow-md w-full max-w-[300px] mx-auto mb-8 border border-gray-100 flex flex-col h-[380px] overflow-hidden">
      ...
      <div className="w-full h-[180px] flex-shrink-0">
        <Image src={image} alt={title} width={300} height={180} className="w-full h-full object-cover rounded-t-xl" />
      </div>
      ...
      <div className="p-4 pb-4 flex flex-col flex-1">
        <div>
          ...
          <p className="text-sm text-gray-500 mb-2 line-clamp-2 overflow-hidden break-words whitespace-normal">{jobTitle}</p>
        </div>
        ...
        <div className="mt-auto flex flex-col gap-3">
          ...
          <div className="flex items-center justify-start">
            <span className="text-md font-semibold text-orange-600 truncate max-w-full">{price}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-medium text-gray-700 truncate max-w-[120px]">{sellerName}</span>
              </div>
            </div>
...
export default function MarketMainPage() {
  ...
  const titleTooLong = formData.name ? formData.name.length > MAX_TITLE_LENGTH : false;
  const descTooLong = formData.description ? formData.description.length > MAX_DESC_LENGTH : false;
  const priceDigits = formData.price ? String(formData.price).replace(/\D/g, '') : '';
  ...
  useEffect(() => {
    const checkClamped = () => {
      try {
        ...
        if (t) {
          setPreviewTitleClamped(t.scrollHeight > t.clientHeight || t.scrollWidth > t.clientWidth);
        }
        if (d) {
          setPreviewDescClamped(d.scrollHeight > d.clientHeight || d.scrollWidth > d.clientWidth);
        }
      } catch {
        // ignore
      }
  ...
  */
  ...
  const [marketItems, setMarketItems] = useState<Array<{ price: string; title: string; jobTitle: string; image: string; sellerName: string; sellerImage: string }>>([]);

  // Default gray placeholder image (data URL SVG) used when API has no image
  const DEFAULT_IMAGE = "data:image/svg+xml;utf8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 600 400'%3E%3Crect width='100%25' height='100%25' fill='%23E5E7EB'/%3E%3C/svg%3E";
    ...
    const fetchProducts = async () => {
      try {
        ...
        const mapped = items.map((p) => {
          ...
          const priceStr = Number.isInteger(priceNum) ? `฿${priceNum}` : `฿${priceNum.toFixed(2)}`;
          return {
            price: priceStr,
            title: truncateText(p.name || 'Untitled Product', MAX_TITLE_LENGTH),
            jobTitle: truncateText(p.description || 'No description provided', MAX_DESC_LENGTH),
            ...
            <div className="flex flex-col md:flex-row h-full">
              ...
              <div className="md:w-1/2 bg-gray-50 p-8 flex items-center justify-center border-r border-gray-200">
                <div className="w-full max-w-[280px]">
                  ...
                  {(titleTooLong || descTooLong || priceTooLong || previewTitleClamped || previewDescClamped) && (
                    <div className="mb-3 text-sm text-red-600">
                      {(titleTooLong || previewTitleClamped) && <div>ชื่อสินค้าตอนนี้ยาวเกิน</div>}
                      {(descTooLong || previewDescClamped) && <div>คำอธิบายยาวเกินกว่าที่จะแสดง โปรดสั้นลง</div>}
                      {priceTooLong && <div>ราคาต้องไม่เกิน 8 หลัก (ตอนนี้ {priceDigits.length} หลัก)</div>}
                    </div>
                  )}
                  <article className="bg-white rounded-xl shadow-md border border-gray-100 flex flex-col" style={{ minHeight: 350 }}>
                    ...
                    <div className="flex-1 flex flex-col p-4 pb-3">
                      <div>
                        <h3 ref={previewTitleRef} className="text-lg font-bold text-gray-900 mb-2 truncate">
                          {formData.name || "Product Name"}
                        </h3>
                        <p ref={previewDescRef} className="text-sm text-gray-500 mb-2 line-clamp-2 overflow-hidden break-words whitespace-normal">
                      ...
                      <div className="mt-auto flex flex-col gap-3">
                        <div className="flex items-center justify-start">
                          <span className="text-sm font-semibold text-orange-600 truncate max-w-full">{formData.price ? `฿${formData.price}` : "฿0"}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="text-sm font-medium text-gray-700 truncate max-w-[120px]">You</span>
                          </div>
